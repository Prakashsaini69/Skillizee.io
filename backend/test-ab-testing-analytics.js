 ...
 * 
 * Usage: node test-ab-testing-analytics.js <courseSlug>
 * Example: node test-ab-testing-analytics.js bundle2
 */

const fetch = require('node-fetch');

// Configuration
const API_BASE_URL = 'https://wchz1q3gvg.execute-api.us-east-1.amazonaws.com/dev';
...
async function fetchCourseId(courseSlug, attemptNumber) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/get-enrollment-course-id?courseSlug=${courseSlug}`);
...
async function runABTestingAnalytics() {
  logHeader('🎯 A/B Testing Analytics Script');
  ...
  log(`API Endpoint: ${API_BASE_URL}/api/get-enrollment-course-id`, 'cyan');
...
module.exports = { runABTestingAnalytics, fetchCourseId };
